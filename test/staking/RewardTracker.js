...
describe("RewardTracker", function () {
  ...
  beforeEach(async () => {
    ...
    rewardDistributor = await deployContract("RewardDistributor", [esGsx.address, rewardTracker.address])
    await rewardDistributor.updateLastDistributionTime()

    await rewardTracker.initialize([gsx.address, esGsx.address], rewardDistributor.address)
  })

  it("inits", async () => {
    expect(await rewardTracker.isInitialized()).eq(true)
    expect(await rewardTracker.isDepositToken(wallet.address)).eq(false)
    expect(await rewardTracker.isDepositToken(gsx.address)).eq(true)
    expect(await rewardTracker.isDepositToken(esGsx.address)).eq(true)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.distributor()).eq(rewardDistributor.address)
    expect(await rewardTracker.rewardToken()).eq(esGsx.address)

    await expect(rewardTracker.initialize([gsx.address, esGsx.address], rewardDistributor.address))
  ...
  it("withdrawToken", async () => {
    ...
    await gsx.mint(rewardTracker.address, 2000)
    await expect(rewardTracker.connect(user0).withdrawToken(gsx.address, user1.address, 2000))
      .to.be.revertedWith("Governable: forbidden")

    await rewardTracker.setGov(user0.address)

    expect(await gsx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user0).withdrawToken(gsx.address, user1.address, 2000)
    expect(await gsx.balanceOf(user1.address)).eq(2000)
  })

  it("stake, unstake, claim", async () => {
    await esGsx.setMinter(wallet.address, true)
    await esGsx.mint(rewardDistributor.address, expandDecimals(50000, 18))
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esGsx per second
    await gsx.setMinter(wallet.address, true)
    await gsx.mint(user0.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(gsx.address, expandDecimals(1000, 18)))
    ...
    await gsx.connect(user0).approve(rewardTracker.address, expandDecimals(1000, 18))
    await rewardTracker.connect(user0).stake(gsx.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, gsx.address)).eq(expandDecimals(1000, 18))
    ...
    await esGsx.mint(user1.address, expandDecimals(500, 18))
    await esGsx.connect(user1).approve(rewardTracker.address, expandDecimals(500, 18))
    await rewardTracker.connect(user1).stake(esGsx.address, expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, gsx.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, esGsx.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, gsx.address)).eq(0)
    expect(await rewardTracker.depositBalances(user1.address, esGsx.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(gsx.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(esGsx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(esGsx.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    await expect(rewardTracker.connect(user0).unstake(esGsx.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user0).unstake(gsx.address, expandDecimals(1001, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await gsx.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).unstake(gsx.address, expandDecimals(1000, 18))
    expect(await gsx.balanceOf(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.totalDepositSupply(gsx.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esGsx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user0).unstake(gsx.address, 1))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount");

    expect(await esGsx.balanceOf(user0.address)).eq(0)
    await rewardTracker.connect(user0).claim(user2.address)
    expect(await esGsx.balanceOf(user2.address)).gt(expandDecimals(1785 + 1190, 18))
    expect(await esGsx.balanceOf(user2.address)).lt(expandDecimals(1786 + 1191, 18))
    ...
    await gsx.mint(user1.address, expandDecimals(300, 18))
    await gsx.connect(user1).approve(rewardTracker.address, expandDecimals(300, 18))
    await rewardTracker.connect(user1).stake(gsx.address, expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(gsx.address)).eq(expandDecimals(300, 18))
    expect(await rewardTracker.totalDepositSupply(esGsx.address)).eq(expandDecimals(500, 18))
    ...
    await expect(rewardTracker.connect(user1).unstake(gsx.address, expandDecimals(301, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");

    await expect(rewardTracker.connect(user1).unstake(esGsx.address, expandDecimals(501, 18)))
      .to.be.revertedWith("RewardTracker: _amount exceeds depositBalance");
    ...
    expect(await esGsx.balanceOf(user2.address)).eq(await rewardTracker.cumulativeRewards(user0.address))
    expect(await esGsx.balanceOf(user3.address)).eq(await rewardTracker.cumulativeRewards(user1.address))

    expect(await gsx.balanceOf(user1.address)).eq(0)
    expect(await esGsx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(gsx.address, expandDecimals(300, 18))
    expect(await gsx.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esGsx.balanceOf(user1.address)).eq(0)
    await rewardTracker.connect(user1).unstake(esGsx.address, expandDecimals(500, 18))
    expect(await gsx.balanceOf(user1.address)).eq(expandDecimals(300, 18))
    expect(await esGsx.balanceOf(user1.address)).eq(expandDecimals(500, 18))
    expect(await rewardTracker.totalDepositSupply(gsx.address)).eq(0)
    expect(await rewardTracker.totalDepositSupply(esGsx.address)).eq(0)

    await rewardTracker.connect(user0).claim(user2.address)
    await rewardTracker.connect(user1).claim(user3.address)

    const distributed = expandDecimals(50000, 18).sub(await esGsx.balanceOf(rewardDistributor.address))
  ...
  it("stakeForAccount, unstakeForAccount, claimForAccount", async () => {
    ...
    await rewardDistributor.setTokensPerInterval("20667989410000000") // 0.02066798941 esGsx per second
    await gsx.setMinter(wallet.address, true)
    await gsx.mint(wallet.address, expandDecimals(1000, 18))

    await rewardTracker.setInPrivateStakingMode(true)
    await expect(rewardTracker.connect(user0).stake(gsx.address, expandDecimals(1000, 18)))
      .to.be.revertedWith("RewardTracker: action not enabled")

    await expect(rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, gsx.address, expandDecimals(1000, 18)))
    ...
    await gsx.connect(wallet).approve(rewardTracker.address, expandDecimals(1000, 18))

    await rewardTracker.connect(user2).stakeForAccount(wallet.address, user0.address, gsx.address, expandDecimals(1000, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(1000, 18))
    expect(await rewardTracker.depositBalances(user0.address, gsx.address)).eq(expandDecimals(1000, 18))
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, esGsx.address, expandDecimals(1000, 18), user1.address))
      .to.be.revertedWith("RewardTracker: forbidden")
    ...
    await expect(rewardTracker.connect(user2).unstakeForAccount(user0.address, gsx.address, expandDecimals(1001, 18), user1.address))
      .to.be.revertedWith("RewardTracker: _amount exceeds stakedAmount")
    ...
    await rewardTracker.connect(user2).unstakeForAccount(user0.address, gsx.address, expandDecimals(100, 18), user1.address)

    expect(await gsx.balanceOf(user1.address)).eq(expandDecimals(100, 18))
    expect(await rewardTracker.stakedAmounts(user0.address)).eq(expandDecimals(900, 18))
    expect(await rewardTracker.depositBalances(user0.address, gsx.address)).eq(expandDecimals(900, 18))
    ...
    expect(await esGsx.balanceOf(user0.address)).eq(0)
    expect(await esGsx.balanceOf(user3.address)).eq(0)

    await rewardTracker.connect(user2).claimForAccount(user0.address, user3.address)

    expect(await rewardTracker.claimable(user0.address)).eq(0)
    expect(await esGsx.balanceOf(user3.address)).gt(expandDecimals(1785, 18))
    expect(await esGsx.balanceOf(user3.address)).lt(expandDecimals(1787, 18))
  })
})
