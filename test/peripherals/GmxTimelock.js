...
describe("GsxTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployContract("GsxTimelock", [
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployContract("GsxTimelock", [
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("GsxTimelock: invalid _buffer")
    ...
    await expect(deployContract("GsxTimelock", [
      ...
      mintReceiver.address,
      1000
    ])).to.be.revertedWith("GsxTimelock: invalid _longBuffer")
  })
  ...
  it("setTokenConfig", async () => {
    ...
    await expect(timelock.connect(wallet).setTokenConfig(
      ...
      0,
      0
    )).to.be.revertedWith("GsxTimelock: token not yet whitelisted")
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployContract("GsxTimelock", [
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
    ...
    await expect(timelock0.connect(user1).setBuffer(3 * 24 * 60 * 60 - 10))
      .to.be.revertedWith("GsxTimelock: buffer cannot be decreased")
  ...
  it("setMaxLeverage", async () => {
    ...
    await expect(timelock.connect(wallet).setMaxLeverage(vault.address, 49 * 10000))
      .to.be.revertedWith("GsxTimelock: invalid _maxLeverage")
  ...
  it("approve", async () => {
    ...
    await expect(timelock.connect(wallet).cancelAction(action0))
      .to.be.revertedWith("GsxTimelock: invalid _action")
  ...
  it("addExcludedToken", async () => {
    const gsx = await deployContract("GSX", [])
    await expect(timelock.connect(user0).addExcludedToken(gsx.address))
      .to.be.revertedWith("GsxTimelock: forbidden")

    expect(await timelock.excludedTokens(gsx.address)).eq(false)
    await timelock.connect(wallet).addExcludedToken(gsx.address)
    expect(await timelock.excludedTokens(gsx.address)).eq(true)
  })
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await timelock.addExcludedToken(gsx.address)
    await expect(timelock.connect(wallet).setInPrivateTransferMode(gsx.address, true))
      .to.be.revertedWith("GsxTimelock: invalid _inPrivateTransferMode")
  ...
  it("setExternalAdmin", async () => {
    ...
    await expect(timelock.connect(wallet).setExternalAdmin(timelock.address, user3.address))
      .to.be.revertedWith("GsxTimelock: invalid _target")
  })
