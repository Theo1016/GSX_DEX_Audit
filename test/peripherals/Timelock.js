...
describe("Timelock", function () {
  ...
  it("updateUsdgSupply", async () => {
    ...
    await usdg.mint(gspManager.address, 1000)

    expect(await usdg.balanceOf(gspManager.address)).eq(1000)
    ...
    expect(await usdg.balanceOf(gspManager.address)).eq(500)
    expect(await usdg.totalSupply()).eq(500)

    await timelock.updateUsdgSupply(2000)

    expect(await usdg.balanceOf(gspManager.address)).eq(2000)
  ...
  it("initGspManager", async () => {
    await expect(timelock.connect(user0).initGspManager())
      .to.be.revertedWith("Timelock: forbidden")

      await gsp.setGov(timelock.address)
      await usdg.setGov(timelock.address)

      expect(await gsp.isMinter(gspManager.address)).eq(false)
      expect(await usdg.vaults(gspManager.address)).eq(false)
      expect(await vault.isManager(gspManager.address)).eq(false)

      await timelock.initGspManager()

      expect(await gsp.isMinter(gspManager.address)).eq(true)
      expect(await usdg.vaults(gspManager.address)).eq(true)
      expect(await vault.isManager(gspManager.address)).eq(true)
  })
  ...
  it("initRewardRouter", async () => {
    await expect(timelock.connect(user0).initRewardRouter())
      ...
      expect(await stakedGspTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await feeGspTracker.isHandler(rewardRouter.address)).eq(false)
      expect(await gspManager.isHandler(rewardRouter.address)).eq(false)

      await timelock.initRewardRouter()

      expect(await stakedGspTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await feeGspTracker.isHandler(rewardRouter.address)).eq(true)
      expect(await gspManager.isHandler(rewardRouter.address)).eq(true)
  })
  ...
  it("processMint", async () => {
    ...
    await gsx.setGov(timelock.address)

    await expect(timelock.connect(user0).processMint(gsx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).processMint(gsx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(user0).signalMint(gsx.address, user1.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: forbidden")

    await timelock.connect(wallet).signalMint(gsx.address, user1.address, expandDecimals(100, 18))
    ...
    await expect(timelock.connect(wallet).processMint(gsx.address, user2.address, expandDecimals(100, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).processMint(gsx.address, user1.address, expandDecimals(101, 18)))
      .to.be.revertedWith("Timelock: action not signalled")

    expect(await gsx.balanceOf(timelock.address)).eq(0)
    expect(await gsx.balanceOf(user1.address)).eq(0)

    await timelock.connect(wallet).processMint(gsx.address, user1.address, expandDecimals(100, 18))
    ...
    const action1 = ethers.utils.solidityKeccak256(["string", "address", "address", "uint256"], ["mint", gsx.address, user1.address, expandDecimals(100, 18)])
  ...
  it("withdrawToken", async () => {
    ...
    await expect(timelock.connect(user0).withdrawToken(gsx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).withdrawToken(gsx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(user0).signalWithdrawToken(gsx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("Timelock: forbidden")

    await timelock.connect(wallet).signalWithdrawToken(gsx.address, bnb.address, user0.address, 100)
    ...
    await expect(timelock.connect(wallet).withdrawToken(gsx.address, dai.address, user0.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gsx.address, bnb.address, user1.address, 100))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gsx.address, bnb.address, user0.address, 101))
      .to.be.revertedWith("Timelock: action not signalled")

    await expect(timelock.connect(wallet).withdrawToken(gsx.address, bnb.address, user0.address, 100))
      .to.be.revertedWith("ERC20: transfer amount exceeds balance")

    await bnb.mint(gsx.address, 100)
    expect(await bnb.balanceOf(user0.address)).eq(0)
    await timelock.connect(wallet).withdrawToken(gsx.address, bnb.address, user0.address, 100)
  ...
  it("setInPrivateTransferMode", async () => {
    ...
    await gsx.mint(user0.address, 100)
    await expect(timelock.connect(user0).setInPrivateTransferMode(gsx.address, true))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setInPrivateTransferMode(gsx.address, true))
      .to.be.revertedWith("BaseToken: forbidden")

    await gsx.setGov(timelock.address)

    expect(await gsx.inPrivateTransferMode()).eq(false)
    await timelock.connect(wallet).setInPrivateTransferMode(gsx.address, true)
    expect(await gsx.inPrivateTransferMode()).eq(true)

    await timelock.connect(wallet).setInPrivateTransferMode(gsx.address, false)
    expect(await gsx.inPrivateTransferMode()).eq(false)

    await timelock.connect(wallet).setInPrivateTransferMode(gsx.address, true)
    expect(await gsx.inPrivateTransferMode()).eq(true)

    await expect(gsx.connect(user0).transfer(user1.address, 100))
      .to.be.revertedWith("BaseToken: msg.sender not whitelisted")

    await timelock.connect(wallet).setInPrivateTransferMode(gsx.address, false)
    expect(await gsx.inPrivateTransferMode()).eq(false)

    await gsx.connect(user0).transfer(user1.address, 100)
  })
  ...
  it("setShortsTrackerAveragePriceWeight", async () => {
    await gspManager.setGov(timelock.address)
    expect(await gspManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setShortsTrackerAveragePriceWeight(1234))
      .to.be.revertedWith("Timelock: forbidden")

    expect(await gspManager.shortsTrackerAveragePriceWeight()).eq(0)
    await timelock.setShortsTrackerAveragePriceWeight(1234)
    expect(await gspManager.shortsTrackerAveragePriceWeight()).eq(1234)
  })

  it("setGspCooldownDuration", async () => {
    await gspManager.setGov(timelock.address)
    expect(await gspManager.gov()).eq(timelock.address)

    await expect(timelock.connect(user0).setGspCooldownDuration(3600))
      .to.be.revertedWith("Timelock: forbidden")

    await expect(timelock.connect(wallet).setGspCooldownDuration(3 * 60 * 60))
      .to.be.revertedWith("Timelock: invalid _cooldownDuration")

    expect(await gspManager.cooldownDuration()).eq(86400)
    await timelock.setGspCooldownDuration(3600)
    expect(await gspManager.cooldownDuration()).eq(3600)
  })
})
