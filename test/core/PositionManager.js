  ...
  beforeEach(async () => {
    ...
    deployTimelock = async () => {
      return await deployContract("Timelock", [
        ...
        ethers.constants.AddressZero, // _gspManager
...
describe("PositionManager next short data calculations", function () {
  ...
  async function debugState(label = "") {
    ...
    const aum = await gspManager.getAum(true)
  ...
  it("PositionManager and GspManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      gspManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      gspManager.shortsTracker(),
    ...
    expect(gspManagerShortTracker, 'gspManager shortsTracker').eq(shortsTracker.address)
  ...
  it("updates global short average prices on position decreases", async () => {
    await shortsTracker.setIsGlobalShortDataReady(true)
    await gspManager.setShortsTrackerAveragePriceWeight(10000)
    expect(await gspManager.shortsTrackerAveragePriceWeight()).to.be.equal(10000)
    ...
    let aumBefore = await gspManager.getAum(true)
    ...
    let aumAfter = await gspManager.getAum(true)
    ...
    aumBefore = await gspManager.getAum(true)
    ...
    aumAfter = await gspManager.getAum(true)
  ...
  it("updates global short average prices on hard liquidation with high borrow fee", async () => {
    ...
    await vault.updateCumulativeFundingRate(dai.address)
