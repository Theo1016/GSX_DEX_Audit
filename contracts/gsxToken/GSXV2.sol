...
contract GSXV2 is IGSX, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "GSX: updated");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "GSX: Invalid hierarchical index");
        _;
    }
    ...
    mapping(address => bool) public isGSXPair;
    ...
    event SetGSXPair(address pair, bool value);
    event PauseContract(bool value);

    modifier onlyWhitelistMint() {
        require(whitelistMint[msg.sender], "onlyWhitelistMint: invalid");
        _;
    }
    ...
    function initialize() public initializer {
        ...
        __GSX_v2_init_unchained();
    }


    function __GSX_v2_init_unchained() internal initializer {
        _name = "GSX";
        _symbol = "GSX";
        _decimals = 9;
    ...
    function __GSX_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 500, 500, 0, 0, 0xb64874f6c35fc3B0D8C4A0bC3e443A734430b982, address(0));
    ...
    function tokenFromReflection(uint256 rAmount) public view returns (uint256) {
        require(rAmount <= _rTotal, "The amount must be less than the total reflection value");
        uint256 currentRate = _getRate();
        return rAmount.div(currentRate);
    }

    function excludeFromReward(address account) public onlyOwner {
        require(!_isExcluded[account], "excludeFromReward:invalid");
    ...
    function includeInReward(address account) external onlyOwner {
        require(_isExcluded[account], "includeInReward: invalid");
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "GSX: Selected account is in blacklist")
    {
        require(_account != address(0), "GSX: Invalid address");
    ...
    function excludeWhitelistedAddress(address _account) public onlyOwner {
        ...
        require(_accountsTier[_account] > 0, "GSX: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "GSX: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "GSX: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "GSX: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "GSX: Owner address is blacklisted")
        preventBlacklisted(spender, "GSX: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "GSX: _msgSender address is blacklisted")
        preventBlacklisted(from, "GSX: from address is blacklisted")
        preventBlacklisted(to, "GSX: to address is blacklisted")
        isRouter(_msgSender())
    {
        require(from != address(0), "BEP20: Transfer money from address zero");
        require(to != address(0), "BEP20: Transfer to zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(_canAction(from, to), "Paused");

        if (from != owner() && to != owner())
            require(amount <= _maxTxAmount, "The transfer amount exceeds maxTxAmount");
    ...
    function _canAction(address from, address to) private view returns (bool) {
        ...
        if (whitelistPause[from] && !isGSXPair[from]) {
            return true;
        }
        return false;
    }
    ...
    function swapTokensForBnb(uint256 tokenAmount) private {        // gsx -> eth 
    ...
    // 交换并进化函数，仅允许所有者调用，并在交换过程中锁定交易
    function swapAndEvolve() public onlyOwner lockTheSwap {
        // 将合同余额分成两半
        uint256 contractBnbBalance = address(this).balance;
        require(contractBnbBalance >= numOfBnbToSwapAndEvolve, "BNB balance is not reach for S&E Threshold");  // BNB余额未达到S&E阈值

        // 设置交易Bnb余额为设定的交换并进化阈值
        contractBnbBalance = numOfBnbToSwapAndEvolve;

        uint256 half = contractBnbBalance.div(2);     // 计算余额的一半
        ...
        // 记录合同当前的BNB余额。
        // 这样我们可以准确捕获交换创建的BNB数量，并且不包括手动发送到合同的任何BNB
        uint256 initialBalance = IGSX(address(this)).balanceOf(msg.sender);      
        // swap BNB for Tokens
        // 交换BNB以获取代币
        ...
        // 我们刚刚交换了多少BNB？
        uint256 newBalance = IGSX(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);
        
        // 授权合同代表所有者转移已交换的代币
        _approve(msg.sender, address(this), swapeedToken);
        IGSX(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {           // eth -> gsx
        // generate the uniswap pair path of token -> wbnb
        address[] memory path = new address[](2);
    ...
    function _takeFees(
        ...
        _takeFee(sender, values.tLiquidity, address(this));     // 添加流动性

        _takeFee(sender, values.tEchoSystem, feeTiers[tierIndex].ecoSystem);   // 生态费用

        
        _takeFee(sender, values.tOwner, feeTiers[tierIndex].owner);  // 分红费用

        
        _takeBurn(sender, values.tBurn);  // 销毁费用
    }
    ...
    //     external
    //     override
    //     preventBlacklisted(account, "GSX: Migrated account is blacklisted")
    // {   
    //     require(migration != address(0), "GSX: Migration is not started");
    //     require(_msgSender() == migration, "GSX: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IGSX(_token).transfer(msg.sender, _amount);
    }
