...
contract Timelock is ITimelock {
    ...
    function initGspManager() external onlyAdmin {
        IGspManager _gspManager = IGspManager(gspManager);

        IMintable gsp = IMintable(_gspManager.gsp());
        gsp.setMinter(gspManager, true);

        IUSDG usdg = IUSDG(_gspManager.usdg());
        usdg.addVault(gspManager);

        IVault vault = _gspManager.vault();
        vault.setManager(gspManager, true);
    }

    function initRewardRouter() external onlyAdmin {
        IRewardRouterV2 _rewardRouter = IRewardRouterV2(rewardRouter);

        IHandlerTarget(_rewardRouter.feeGspTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(_rewardRouter.stakedGspTracker()).setHandler(rewardRouter, true);
        IHandlerTarget(gspManager).setHandler(rewardRouter, true);
    }
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = IGspManager(gspManager).usdg();
        uint256 balance = IERC20(usdg).balanceOf(gspManager);

        IUSDG(usdg).addVault(address(this));

        if (usdgAmount > balance) {
            uint256 mintAmount = usdgAmount.sub(balance);
            IUSDG(usdg).mint(gspManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IUSDG(usdg).burn(gspManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IGspManager(gspManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }

    function setGspCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        require(_cooldownDuration < 2 hours, "Timelock: invalid _cooldownDuration");
        IGspManager(gspManager).setCooldownDuration(_cooldownDuration);
    }
