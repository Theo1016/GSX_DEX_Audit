...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    function getIncreasePositionRequestPath(bytes32 _key) public override  view returns (address[] memory) {
        IncreasePositionRequest memory request = increasePositionRequests[_key];
        return request.path;
    }

    function getDecreasePositionRequestPath(bytes32 _key) public override  view returns (address[] memory) {
        DecreasePositionRequest memory request = decreasePositionRequests[_key];
        return request.path;
    }
    ...
    function _callRequestCallback(
        ...
        bool success;
        try IPositionRouterCallbackReceiver(_callbackTarget).gsxPositionCallback{ gas: _gasLimit }(_key, _wasExecuted, _isIncrease) {
            success = true;
        } catch {}
